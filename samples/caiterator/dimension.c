...
rb_di_initialize_copy (VALUE self, VALUE other)
{
  ...
  rref = rb_ivar_get(other, rb_intern("@reference"));
  rker = rb_obj_clone(rb_ivar_get(other, rb_intern("@kernel")));

  *is = *io;

  TypedData_Get_Struct(rker, CArray, &carray_data_type, is->kernel);
