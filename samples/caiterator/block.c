...
ca_bi_kernel_at_index (void *it, ca_size_t *idx, CArray *ref)
{
  ...
  ca_block_sync_base_offset(kernel);
  if ( kernel->mask ) {
    ca_block_sync_base_offset((CABlock *)(kernel->mask));
  }
...
ca_bi_kernel_move_to_index (void *it, ca_size_t *idx, CArray *kern)
{
  ...
  for (i=0; i<kernel->ndim; i++) {
    j = idx[i];
    CA_CHECK_INDEX(j, bit->dim[i]);
    /* Mirror ca_bi_kernel_at_index semantics: position[j] is
       bit->kernel->start[i] + j*dim, not j*dim.  Using `=` with j*dim
       would silently drop a non-zero initial offset (e.g. a.blocks(2..4)),
       so #each would yield blocks starting at 0 instead of 2. */
    kernel->start[i] = bit->kernel->start[i] + j * kernel->dim[i];
    if ( kernel->mask ) {
      ((CABlock*)(kernel->mask))->start[i] =
        bit->kernel->start[i] + j * kernel->dim[i];
    }
  }
  ca_block_sync_base_offset(kernel);
  if ( kernel->mask ) {
...
{
  ca_bi_setup(self, rb_ivar_get(other, rb_intern("@reference")),
