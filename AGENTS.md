...
| an operation between arrays of different rank | expecting NumPy's trailing-axis alignment | it does not happen, and the mismatch is raised. Declare the axis instead: `a + c[:_, nil]`. `:_` inserts a size-1 axis, which then stretches to match |
